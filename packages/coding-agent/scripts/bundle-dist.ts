...
const outDir = configuredOutDir ? path.resolve(configuredOutDir) : path.join(packageDir, "dist");
...
async function cleanBundleOutputs(): Promise<void> {
	// dist/ is shared with the desktop binary; Android passes a dedicated
	// artifact directory through OMP_BUNDLE_OUTDIR.
	let entries: string[];
	try {
...
async function main(): Promise<void> {
	...
	process.stdout.write(
		`Bundled coding-agent CLI to ${path.relative(packageDir, cliPath)} (${formatBytes(stat.size)}) in ${elapsedMs.toFixed(0)}ms\n`,
	);
}
